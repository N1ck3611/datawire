...
const UserSettings = () => {
  ...
  const handleUnlinkDiscord = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token')
      const response = await fetch(`${API_BASE}/api/auth/unlink-discord`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (data.success) {
        ...
        setUser({ ...user, discordId: null, global_name: null })
      } else {
        setDiscordError(data.error || 'Failed to unlink Discord account')
      }
    } catch (error) {
      setDiscordError('Network error. Please try again.')
    } finally {
            ...
            <div className="space-y-4">
              {user?.discordId ? (
                <>
                  ...
                  >
                    {unlinkingDiscord ? 'Unlinking...' : 'Unlink Discord Account'}
                  </Button>
                </>
              ) : (
