    ...
    if (searchType === 'usernames') {
      if (searchQuery.includes('github')) {
        ...
        simulatedResults.push({
          ...
          url: githubUrl,
          snippet: `GitHub user profile for ${query}`,
          fetched: true
        });
        ...
        try {
          ...
          if (githubData) {
            ...
            simulatedResults.push({
              ...
              url: githubUrl,
              snippet: `Found: ${githubData.name || 'No name'}, ${githubData.email || 'No email'}, ${githubData.public_repos} repos, ${githubData.followers} followers`,
              extracted: true,
              data: githubData
            });
          }
      ...
      if (searchQuery.includes('twitter')) {
        ...
        simulatedResults.push({
          ...
          url: twitterUrl,
          snippet: `Twitter/X profile for username ${query}`,
          fetched: true
        });
      ...
      if (searchQuery.includes('reddit')) {
        ...
        simulatedResults.push({
          ...
          url: redditUrl,
          snippet: `Reddit user profile for ${query}`,
          fetched: true
        });
    ...
    if (searchType === 'emails') {
      if (searchQuery.includes('breach') || searchQuery.includes('leak')) {
        simulatedResults.push({
          ...
          snippet: `Check if email ${query} has been compromised in data breaches`,
          fetched: true
        });
      ...
      if (searchQuery.includes('github')) {
        simulatedResults.push({
          ...
          snippet: `GitHub search for email ${query}`,
          fetched: true
        });
      }
    }
    ...
    if (searchType === 'ipAddresses') {
      if (searchQuery.includes('abuseipdb')) {
        simulatedResults.push({
          ...
          snippet: `IP address ${query} abuse report`,
          fetched: true
        });
      ...
      if (searchQuery.includes('virustotal')) {
        simulatedResults.push({
          ...
          snippet: `VirusTotal analysis for IP ${query}`,
          fetched: true
        });
      }
      ...
      if (searchQuery.includes('shodan')) {
        simulatedResults.push({
          ...
          snippet: `Shodan Internet search for IP ${query}`,
          fetched: true
        });
      }
    }
    ...
    if (searchType === 'domains') {
      if (searchQuery.includes('whois')) {
        simulatedResults.push({
          ...
          snippet: `WHOIS information for domain ${query}`,
          fetched: true
        });
      ...
      if (searchQuery.includes('dns')) {
        simulatedResults.push({
          ...
          snippet: `DNS records lookup for domain ${query}`,
          fetched: true
        });
      }
    }
...
async function executeAIOsintInvestigation(identifiers, userId, investigationId) {
  ...
  try {
    ...
    const systemPrompt = `You are an elite AI OSINT orchestrator for DataWire.cc with advanced intelligence gathering capabilities based on industry best practices from OpenOSINT, osint-methodology-vault, and agentic-osint-agent.
...
1. REQUIREMENTS - Analyze the target identifiers and define investigation scope
2. COLLECTION - Execute ALL relevant tools for each identifier type systematically
3. PROCESSING - Normalize, validate, and deduplicate collected data
4. ANALYSIS - Cross-reference data points, identify patterns, and detect connections
5. DISSEMINATION - Generate actionable intelligence with confidence levels

INTELLIGENCE GATHERING STRATEGY:
1. For EACH identifier provided, execute ALL relevant tools for that identifier type
2. Cross-reference data across multiple sources to build comprehensive intelligence profiles
3. Prioritize tools based on data richness, reliability, and source diversity
4. Example: For emails, run email_breach_search, email_osint, and any other email-related tools
5. Example: For usernames, run username_search, social_media_lookup, and platform-specific tools
6. Example: For domains, run domain_intel, WHOIS lookups, DNS enumeration, and subdomain discovery
7. Example: For IPs, run ip_intel, geolocation, abuse checks, and infrastructure analysis

PIVOT PLAYBOOKS:
- Email → Username extraction → Social media search → Profile analysis
- Username → Platform enumeration → Cross-platform correlation → Identity clustering
- Domain → WHOIS → DNS → Subdomain enumeration → Infrastructure mapping
- IP → Geolocation → ISP → Abuse reputation → Network analysis
- Phone → Carrier → Location → Messaging app registration → Identity linking
- Discord ID → User lookup → Connected accounts → Username extraction → Social media

CRITICAL RULES:
- Use ONLY the exact identifier values provided - NEVER extract substrings or modify identifiers
- If an email is provided, use the FULL email - do NOT extract just the username part
- If a phone number is provided, use it exactly as given - normalize format but don't change the number
- Generate comprehensive tool execution plans that maximize data collection
- Include fallback tools if primary tools fail
- Prioritize tools that provide actual data over those that return search URLs
- Execute tools in parallel when possible to speed up investigation
- Cross-reference findings across different sources to validate intelligence

ANTI-HALLUCINATION PROTOCOLS:
- NEVER fabricate findings - only report actual data from tool executions
- Always cite the specific tool and source for each piece of intelligence
- If a tool fails, report the failure rather than inventing results
- Validate data formats before including in intelligence reports
- Flag uncertain findings with appropriate confidence levels

OUTPUT FORMAT:
Return ONLY a JSON array of tool executions:
[{"tool": "tool_name", "identifier": "exact_value", "priority": "high|medium|low"}]

Be thorough and methodical. Quality intelligence requires comprehensive data collection from all available sources. Follow the intelligence cycle systematically and apply pivot playbooks to maximize discovery.`;
    
    const userPrompt = `INVESTIGATION TARGET:
${JSON.stringify(identifiers, null, 2)}

INSTRUCTIONS:
1. Analyze each identifier type
2. For EACH identifier value, determine ALL relevant tools
3. Create a comprehensive execution plan
4. Use the EXACT identifier values as provided - no modifications
5. Prioritize high-value intelligence sources

Generate the complete tool execution plan as JSON only.`;
  ...
  try {
    ...
    // Generate enhanced executive summary with intelligence assessment
    ...
    state.report = {
      ...
      nextSteps: generateNextSteps(discoveredData, connections, leads),
      metadata: {
        investigationId,
        timestamp: new Date().toISOString(),
        ...
        confidenceScore: calculateOverallConfidence(connections, leads)
      }
    };
...
async function handleAPI(request, path, url, ctx) {
  ...
  if (path.startsWith('/api/user/public/') && method === 'GET') {
    try {
      ...
      console.log('[Public Profile] User removeVideoAudio value:', user.removeVideoAudio, 'Type:', typeof user.removeVideoAudio);
      ...
      const publicUser = {
        ...
        removeVideoAudio: user.removeVideoAudio,
        plan: user.plan,
  ...
  // Toggle remove video audio
  if (path === '/api/user/remove-video-audio' && method === 'PUT') {
    try {
      const { remove } = await request.json();
      
      if (typeof remove !== 'boolean') {
        return new Response(JSON.stringify({ error: 'Remove value must be boolean' }), {
          status: 400,
          headers: { ...CORS_HEADERS, 'Content-Type': 'application/json' }
        });
      }
      ...
      await firebasePatch(`/users/${safeUserId}`, {
        removeVideoAudio: remove,
        updatedAt: new Date().toISOString()
      });
      
      return new Response(JSON.stringify({ 
        success: true, 
        message: 'Remove audio setting updated successfully' 
      }), {
        headers: { ...CORS_HEADERS, 'Content-Type': 'application/json' }
      });
    } catch (error) {
  ...
  if (path === '/api/ai-osint-search' && method === 'POST') {
    try {
      ...
      investigationState.set(investigationId, initialState);
